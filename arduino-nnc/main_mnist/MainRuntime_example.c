...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < 8; i++) {
		...
		fclose(input0);
#else
		FILE* input0; int rc = os_fopen(buf, "rb");
		if (rc== 0) { printf("File [%s] Not.Found\n", buf); while (1); }
		int input_read_size0 = os_fread(idbf, sizeof(float), NNABLART_MAINRUNTIME_INPUT0_SIZE, input0);
		assert(input_read_size0 == NNABLART_MAINRUNTIME_INPUT0_SIZE);
		os_fclose(input0);
#endif
