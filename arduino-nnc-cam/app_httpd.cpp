...
static esp_err_t pgm_handler(httpd_req_t *req)
{
    ...
    size_t fb_len = 0;
    if (fb->format == PIXFORMAT_JPEG || fb->format == PIXFORMAT_GRAYSCALE) {
        fb_len = fb->len;
        //res = httpd_resp_send(req, (const char *)fb->buf, fb->len);
        res = httpd_resp_send(req, (const char *)fb->buf, 160*120+15);
    } else {
...
void startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &jpg_uri); // Add
        httpd_register_uri_handler(camera_httpd, &pgm_uri); // Add
    }
